...
class GameModeConfig(BaseModel):
    ...
    slots: Dict[str, List[float]] = Field(..., description="Card slot positions [x1, y1, x2, y2, angle]")

    @field_validator('slots')
    @classmethod
    def validate_slots(cls, v):
        required_slots = ['hero_hole_1', 'hero_hole_2', 'flop_1', 'flop_2', 'flop_3', 'turn', 'river']
        for slot in required_slots:
            if slot not in v:
                raise ValueError(f"Required slot '{slot}' missing from game mode configuration")
        return v


class DetectionConfig(BaseModel):
    """Configuration for card detection."""
    confidence_threshold: float = Field(0.1, ge=0.0, le=1.0, description="Minimum confidence for card detection")
    template_dir: str = Field("templates", description="Directory containing card templates")
    max_image_dimension: int = Field(1000, description="Maximum dimension for image resizing")
...
class AppConfig(BaseModel):
    ...
    game_modes: Dict[str, GameModeConfig] = Field(..., description="Available game mode configurations")
    detection: DetectionConfig = Field(default_factory=DetectionConfig, description="Detection settings")
    capture: CaptureConfig = Field(default_factory=CaptureConfig, description="Capture settings")
