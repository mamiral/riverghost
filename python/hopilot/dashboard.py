    ...
    def execute(self):
        with self.dashboard.speed_lock:
            ...
            if not self.dashboard.auto_save:
                ...
                self.dashboard.saved_this_round = {
                    i: False for i in range(len(self.dashboard.bboxes))
                }
...
class CropBBoxesCommand(Command):
    ...
    def execute(self):
        ...
        if frame is not None:
            ...
            bbox_files = [
                f for f in files if f.startswith("bbox") and f.endswith(".png")
            ]
            numbers = []
            for f in bbox_files:
                try:
            ...
            for i, (x1, y1, x2, y2, angle) in enumerate(self.dashboard.bboxes_hole):
                ...
                hole_files = [
                    f
                    for f in files
                    if f.startswith(f"hole_{hole_prefix}_")
                    and f.endswith(".png")
                    and "_orig" not in f
                ]
                numbers_hole = []
                ...
                crop = rotated_sub[
                    int(rel_center[1] - size[1] / 2) : int(rel_center[1] + size[1] / 2),
...
class Dashboard:
    ...
    def set_game_mode(self, game_mode):
        ...
        self.bboxes = [
            [int(coord) for coord in bbox] for bbox in self.layout.get_board_bboxes()
        ]
        self.bboxes_hole = [
            [int(coord) for coord in bbox] for bbox in self.layout.get_hole_bboxes()
        ]
        ...
        if self.round_active:
            ...
            for i, bbox in enumerate(self.bboxes):
                ...
                if self.is_bbox_steady(frame, bbox):
                    ...
                    suffix = (
                        self.bbox_suffixes[i]
                        if i < len(self.bbox_suffixes)
                        else f"bbox{i}"
                    )
            ...
            for i, (x1, y1, x2, y2, angle) in enumerate(self.bboxes_hole):
                ...
                os.makedirs("recordings/screenshots/auto_capture", exist_ok=True)
                suffix = (
                    self.bbox_hole_suffixes[i]
                    if i < len(self.bbox_hole_suffixes)
                    else f"hole{i}"
                )
                self.auto_save_counter += 1
                path = f"recordings/screenshots/auto_capture/{self.auto_save_counter}_{suffix}.png"
    ...
    def display_cards(self, assignments, advice, image_path=None):
        ...
        if self.video_mode:
            ...
            self.screen.blit(
                self.font.render(text, True, (0, 0, 0)), (605, self.button_y + 5)
            )

            # Speed up
            pygame.draw.rect(self.screen, (0, 255, 0), self.speed_up_rect)
            self.screen.blit(
                self.font.render("Speed +", True, (0, 0, 0)), (715, self.button_y + 5)
            )

            # Speed down
            pygame.draw.rect(self.screen, (255, 0, 0), self.speed_down_rect)
            self.screen.blit(
                self.font.render("Speed -", True, (0, 0, 0)), (605, self.button_y + 45)
            )
            ...
            if self.replay_mode:
                ...
                self.screen.blit(
                    self.font.render(text, True, (0, 0, 0)), (715, self.button_y + 45)
                )
            else:
            ...
            # Speed display
            speed_text = (
                f"Replay Speed: {self.speed:.1f}x"
                if self.replay_mode
                else f"Speed: {self.speed:.1f}x"
            )
        ...
        self.screen.blit(
            self.font.render("Screenshot", True, (0, 0, 0)), (715, self.button_y + 85)
        )
        ...
        if self.video_mode:
            ...
            self.screen.blit(
                self.font.render(text, True, (0, 0, 0)), (605, self.button_y + 85)
            )

            # Crop BBoxes button
            pygame.draw.rect(self.screen, (255, 255, 0), self.crop_rect)
            self.screen.blit(
                self.font.render("Crop BBoxes", True, (0, 0, 0)),
                (605, self.button_y + 125),
            )
            ...
            self.screen.blit(
                self.font.render(text, True, (0, 0, 0)), (715, self.button_y + 125)
            )
        ...
        if self.dir_mode:
            ...
            self.screen.blit(
                self.font.render("Prev", True, (0, 0, 0)), (605, self.button_y + 125)
            )

            # Next button
            pygame.draw.rect(self.screen, (255, 165, 0), self.next_rect)
            self.screen.blit(
                self.font.render("Next", True, (0, 0, 0)), (715, self.button_y + 125)
            )
    ...
    def run(
        self,
        card_assignments_func,
        advice_func,
        image_path_func=None,
        prev_func=None,
        next_func=None,
        dir_mode=False,
        video_mode=False,
        ...
        self.logger.info(
            f"Starting Dashboard run with game_mode: {game_mode}, dir_mode: {dir_mode}, video_mode: {video_mode}, replay_mode: {replay_mode}"
        )
        ...
        while running:
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    ...
                    if self.video_mode and self.is_mouse_inside(
                        self.pause_rect, event.pos
                    ):
                        toggle_pause_cmd.execute()
                    elif self.video_mode and self.is_mouse_inside(
                        self.speed_up_rect, event.pos
                    ):
                        speed_up_cmd.execute()
                    elif self.video_mode and self.is_mouse_inside(
                        self.speed_down_rect, event.pos
                    ):
                        speed_down_cmd.execute()
                    elif self.video_mode and self.is_mouse_inside(
                        self.slow_toggle_rect, event.pos
                    ):
                        toggle_slow_cmd.execute()
                    elif self.is_mouse_inside(self.screenshot_rect, event.pos):
                        screenshot_cmd.execute()
                    elif self.video_mode and self.is_mouse_inside(
                        self.recording_rect, event.pos
                    ):
                        toggle_recording_cmd.execute()
                    elif self.video_mode and self.is_mouse_inside(
                        self.crop_rect, event.pos
                    ):
                        crop_cmd.execute()
                    elif self.video_mode and self.is_mouse_inside(
                        self.auto_save_rect, event.pos
                    ):
                        toggle_auto_save_cmd.execute()
                    elif (
                        self.dir_mode
                        and self.is_mouse_inside(self.prev_rect, event.pos)
                        and self.prev_func
                    ):
                        self.prev_func()
                    elif (
                        self.dir_mode
                        and self.is_mouse_inside(self.next_rect, event.pos)
                        and self.next_func
                    ):
                        self.next_func()
                ...
                if not self.paused:
                    ...
                    if ret:
                        ...
                        for x1, y1, x2, y2 in self.bboxes:
                            cv2.rectangle(
                                frame_copy, (x1, y1), (x2, y2), (255, 0, 0), 2
                            )
                        ...
                        cv2.resizeWindow(
                            "Captured Frame", frame.shape[1], frame.shape[0]
                        )
                        cv2.waitKey(1)
