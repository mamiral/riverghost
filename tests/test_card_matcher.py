...
import numpy as np
from hopilot.card_matcher import CardMatcher, classify_suite, detect_card_color
...
class TestCardMatcher:
    ...
    def test_suite_recognition_accuracy(self, card_matcher, test_data_dir, suite_name):
        ...
        if total_predictions > 0:
            ...
            assert (
                accuracy >= 0.8
            ), f"Suite {suite_name} recognition accuracy too low: {accuracy:.2f}"
    ...
    def test_rank_recognition(self, card_matcher, test_data_dir):
        ...
        for expected_suite, expected_rank in test_cases:
            ...
            card_files = list(
                (test_data_dir / expected_suite / expected_rank).glob("*.png")
            )
            if not card_files:
                continue
            ...
            ), f"Failed to recognize rank for {full_path}"
            assert (
                result["rank_score"] > 0
            ), f"Rank score should be positive for {full_path}"
