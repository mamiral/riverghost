...
- [x] T001 Create project structure per plan.md: `python/aof_gto_browser_ii/shared/` with `domain/`, `adapters/`, `exceptions/` subdirectories
- [x] T002 [P] Create test directory structure: `tests/aof_gto_browser_ii/` with subdirs for test files
- [x] T003 [P] Create `python/aof_gto_browser_ii/__init__.py` (placeholder for package exports)
- [x] T004 [P] Create `python/aof_gto_browser_ii/shared/domain/__init__.py` (placeholder for domain exports)
- [x] T005 [P] Create `python/aof_gto_browser_ii/shared/adapters/__init__.py` (placeholder for adapter exports)
- [x] T006 [P] Create `python/aof_gto_browser_ii/shared/exceptions/__init__.py` (placeholder for exception exports)
- [x] T007 [P] Create `tests/aof_gto_browser_ii/conftest.py` with shared fixtures (Card, Hand, Board instances for reuse)
...
- [x] T008 Create `python/aof_gto_browser_ii/shared/exceptions/validation_errors.py` with custom exception classes:
...
- [x] T009 [P] [US1] Implement Rank enum (IntEnum, 2-14 where A=14) with properties: `char` for single-char representation ("A", "K", "Q", "J", "T", "2"-"9")
- [x] T010 [P] [US1] Implement Suit enum (str Enum, s/h/d/c) with property: `full_name` for full names ("Spades", "Hearts", "Diamonds", "Clubs")
- [x] T011 [US1] Implement Card frozen dataclass with `rank: Rank` and `suit: Suit` fields in `python/aof_gto_browser_ii/shared/domain/card.py`
- [x] T012 [US1] Implement `Card.from_string(s: str) -> Card` parser accepting:
...
- [x] T013 [US1] Implement `Card.to_string() -> str` returning normalized shorthand like "As", "Kh"
- [x] T014 [US1] Implement `Card.__str__()` and `Card.__repr__()` for string representation
- [x] T015 [US1] Implement `Card.is_ace() -> bool` helper method
- [x] T016 [P] [US1] Create `tests/aof_gto_browser_ii/test_card.py` with comprehensive tests:
...
- [x] T017 [P] [US1] Implement Hand frozen dataclass with `card1: Card` and `card2: Card` fields in `python/aof_gto_browser_ii/shared/domain/hand.py`
- [x] T018 [US1] Implement `Hand.__post_init__()` validation raising ValueError if card1 == card2 (no duplicate cards)
- [x] T019 [US1] Implement `Hand.from_strings(card1_str: str, card2_str: str) -> Hand` parser delegating to Card.from_string()
- [x] T020 [US1] Implement `Hand.from_tuple((str, str)) -> Hand` parser for tuple input
- [x] T021 [US1] Implement `Hand.to_strings() -> tuple[str, str]` returning ("As", "Kd") format for DTO transport
- [x] T022 [US1] Implement `Hand.to_shorthand() -> str` returning standardized notation:
...
- [x] T023 [US1] Implement hand classification methods:
...
- [x] T024 [US1] Implement `Hand.__str__()` returning to_shorthand(), `Hand.__repr__()` returning clear representation
- [x] T025 [P] [US1] Create `tests/aof_gto_browser_ii/test_hand.py` with comprehensive tests:
...
- [x] T026 [P] [US1] Implement HandRange frozen dataclass with:
...
- [x] T027 [US1] Implement `HandRange.from_shorthand(notation: str) -> HandRange` parser with 3-phase algorithm:
...
- [x] T028 [US1] Implement `HandRange.to_shorthand() -> str` returning original notation for serialization
- [x] T029 [US1] Implement `HandRange.to_strings() -> List[tuple[str, str]]` returning card pair list for DTO transport
- [x] T030 [US1] Implement HandRange query methods:
...
- [x] T031 [US1] Implement `HandRange.union(other: HandRange) -> HandRange` static method combining ranges and deduplicating
- [x] T032 [US1] Implement `HandRange.intersection(other: HandRange) -> HandRange` static method returning common hands
- [x] T033 [US1] Implement `HandRange.__str__()` and `HandRange.__repr__()` for representation
- [x] T034 [P] [US1] Create `tests/aof_gto_browser_ii/test_hand_range.py` with comprehensive tests:
...
- [x] T035 [P] [US1] Implement Board frozen dataclass with `cards: List[Card]` field (0-5 cards in street order) in `python/aof_gto_browser_ii/shared/domain/board.py`
- [x] T036 [US1] Implement `Board.__post_init__()` validation:
  - Raise ValueError if len(cards) > 5
  - Raise ValueError if duplicate cards detected
- [x] T037 [US1] Implement `Board.from_strings(card_strings: List[str]) -> Board` parser delegating to Card.from_string()
- [x] T038 [US1] Implement `Board.to_strings() -> List[str]` returning card string list
- [x] T039 [US1] Implement street detection methods:
...
- [x] T040 [US1] Implement card accessors:
...
- [x] T041 [US1] Implement `Board.__str__()` returning space-separated cards, `Board.__repr__()` for representation
- [x] T042 [P] [US1] Create `tests/aof_gto_browser_ii/test_board.py` with comprehensive tests:
...
- [x] T043 [P] [US1] Implement EquityResult frozen dataclass with fields in `python/aof_gto_browser_ii/shared/domain/equity_result.py`:
  - `hand: Hand`
...
- [x] T044 [US1] Implement `EquityResult.__post_init__()` validation:
...
- [x] T045 [US1] Implement display properties:
...
- [x] T046 [US1] Implement `EquityResult.from_monte_carlo(hand: Hand, wins: int, draws: int, losses: int, total_simulations: int) -> EquityResult` factory method:
  ...
  - Return EquityResult
- [x] T047 [US1] Implement `EquityResult.__str__()` and `EquityResult.__repr__()` for representation
- [x] T048 [P] [US1] Create `tests/aof_gto_browser_ii/test_equity_result.py` with comprehensive tests:
...
- [x] T049 [P] [US1] Implement Bet frozen dataclass with `amount_bb: float` field (amount in big blinds) in `python/aof_gto_browser_ii/shared/domain/bet.py`
- [x] T050 [US1] Implement `Bet.__post_init__()` validation:
...
- [x] T051 [US1] Implement `Bet.is_zero() -> bool` returning False (always, since amount > 0)
- [x] T052 [US1] Implement `Bet.is_all_in(stack_bb: float) -> bool` returning True if amount_bb >= stack_bb
- [x] T053 [US1] Implement `Bet.display_value() -> str` formatting amount as "2.5 BB" or "0.50 BB" for UI display
- [x] T054 [US1] Implement `Bet.__str__()` and `Bet.__repr__()` for representation
- [x] T055 [P] [US1] Create `tests/aof_gto_browser_ii/test_bet.py` with comprehensive tests:
...
- [x] T056 [P] [US2] Implement CardAdapter class in `python/aof_gto_browser_ii/shared/adapters/card_adapter.py` with:
...
- [x] T057 [US2] Implement `CardAdapter.to_pokerkit(card: Card) -> str`:
...
- [x] T058 [US2] Implement `CardAdapter.from_pokerkit(s: str) -> Card`:
  - Parse PokerKit string format ("As", "Kh", etc.) back to domain Card
  - Delegate to Card.from_string() internally
  - Return Card object
- [x] T059 [US2] Implement `CardAdapter.to_pokerkit_hand(hand: Hand) -> tuple[str, str]`:
  - Convert domain Hand to PokerKit hand format (tuple of 2 card strings)
  - Return (card1_string, card2_string)
- [x] T060 [US2] Implement `CardAdapter.from_pokerkit_hand(c1: str, c2: str) -> Hand`:
  - Parse 2 PokerKit card strings back to domain Hand
  - Delegate to Hand.from_strings() internally
  - Return Hand object
- [x] T061 [P] [US2] Create `tests/aof_gto_browser_ii/test_card_adapter.py` with comprehensive tests:
...
- [x] T062 Update `python/aof_gto_browser_ii/__init__.py` to export all public API:
...
- [x] T063 Update `python/aof_gto_browser_ii/shared/domain/__init__.py` to export domain classes
- [x] T064 Update `python/aof_gto_browser_ii/shared/adapters/__init__.py` to export CardAdapter
- [x] T065 Update `python/aof_gto_browser_ii/shared/exceptions/__init__.py` to export exception classes

### Test Coverage & Validation

- [x] T066 Run full test suite from repository root:
  - Command: `python -m pytest tests/aof_gto_browser_ii/ -v`
  - Verify all tests pass
- [x] T067 Generate coverage report:
...
- [x] T069 Validate success criteria completion:
...
- [x] T072 Run complete test suite one final time:
  - Command: `python -m pytest tests/aof_gto_browser_ii/ -v --tb=short`
  - All tests must pass
  - Coverage must be >= 80%
- [x] T073 Verify no type errors (optional but recommended):
  - If mypy available: `mypy python/aof_gto_browser_ii/shared/`
  - Fix any type issues
- [x] T074 Commit code changes and push to branch:
...
- [x] **SC-001**: All immutable domain models (Card, Hand, HandRange, Board, EquityResult, Bet) can be created, parsed from strings, serialized back to strings, and cannot be mutated after creation (attempt to modify raises AttributeError)
  - Verified by: T016, T025, T034, T042, T048, T055 (immutability tests)

- [x] **SC-002**: HandRange parser correctly expands complex shorthand notation like "AKs+, QQ+, A5s-A2s" and contains() method correctly identifies membership for all 1326 possible hands in < 10ms
  - Verified by: T034 (HandRange tests with performance check)

- [x] **SC-003**: EquityResult objects correctly validate that win_prob + draw_prob + loss_prob sums to 1.0 ± 0.01, raising ValueError with descriptive message for violations
  - Verified by: T048 (EquityResult validation tests)

- [x] **SC-004**: Bet value type validates that amount_bb > 0 and is finite (not NaN/Infinity), raising ValueError for invalid inputs
  - Verified by: T055 (Bet validation tests)

- [x] **SC-005**: CardAdapter converts domain Card objects to PokerKit string format ("As", "Kh", etc.) and back with 100% accuracy for all 52 cards in < 1μs per conversion (round-trip: Card → string → Card equals original)
  - Verified by: T061 (CardAdapter round-trip tests with performance check)

- [x] **SC-006**: All domain model classes are hashable and work correctly in sets and as dictionary keys (e.g., {Card.from_string("As"): ...} or hand_set.add(hand))
  - Verified by: T016, T025, T034, T042, T048, T055 (hashability tests in each module's test suite)

- [x] **SC-007**: Test coverage for domain models (Card, Hand, HandRange, Board, EquityResult, Bet, CardAdapter) reaches 80% line coverage (measured with `coverage.py` or equivalent) with unit tests for parsing, validation, edge cases, immutability, and round-trip serialization
  - Verified by: T067 (coverage report generation), T068 (fill gaps to reach 80%)

- [x] **SC-008**: All public APIs have docstrings explaining purpose, parameters, return values, and example usage; README documents domain model architecture and design choices
  - Verified by: T070-T071 (docstring addition, documentation)

- [x] **SC-009**: Domain models integrate with CardAdapter without type errors: CardAdapter methods accept Card objects and return strings; parsing methods handle all valid poker notation formats
  - Verified by: T061 (CardAdapter tests with Card objects), T069 (type error check)

- [x] **SC-010**: Development team reviews and approves domain model API contracts; no breaking changes expected for Phase 1.2+ development
